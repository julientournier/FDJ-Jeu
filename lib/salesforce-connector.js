...
var Salesforce = function(config) {
    ...
    return {
         ...
         */
        getLocationsbkup: function(cb) {
            conn.query("SELECT Id, Name, latitude__c, longitude__c FROM Account", function(err, result) {
                if (err) {
                    console.log(err);
        ...
        getLocations: function(cb) {
            conn.query("SELECT Id, Name, latitude__c, longitude__c FROM Planned_Store__c WHERE longitude__c != null AND latitude__c != null", function(err, result) {
                if (err) {
                    console.log(err);
                    throw "Cannot retrieve Stores";
                }
                console.log(result);
                var locations = [];
                result.records.forEach(function(el) {
         ...
         */
        saveScorebkup: function(account_id, patron_name, points, cb) {
        ...
        saveScore: function(account_id, patron_name, points, cb) {
            this.ensurePatronExists(patron_name, function(patron_id) {
                conn.sobject("score__c").create({ patron__c : patron_id, store__c : account_id, points__c: points }, function(err, ret) {
                    if (err || !ret.success) {
                        console.log(err, ret);
                        throw "Cannot save Score";
                    }
         ...
         */
        getHighScoresByLocationbkup: function(account_id, cb) {
        ...
        getHighScoresByLocation: function(account_id, cb) {
            conn.query("SELECT points__c, patron__r.Name FROM score__c WHERE store__c = '"+escape(account_id)+"' ORDER BY points__c DESC LIMIT 5", function(err, result) {
                if (err) {
                    console.log(err);
                    throw "Cannot retrieve Scores";
                }
